...
for file in files_split:
	...
	b = cell_list[i]
	c = str(b)+'arcsec'
	out = file.split('.ms.split.cal')[0]+'_N'+'.png'
	sdimaging(infiles=[file],outfile=out, overwrite=True, spw=spw, gridfunction='SF',convsupport=6,imsize=[a,a],cell=[c,c],stokes='I',veltype='radio',outframe='lsrk')
	i=i+1
sdimaging(infiles=files_split,outfile=path+'collectedsdimaging_N.png', overwrite=True, spw=spw, gridfunction='SF',convsupport=6,imsize=[101,101],cell=['6.98arcsec','6.98arcsec'],stokes='I',veltype='radio',outframe='lsrk')
